...
class _Base:
    ...
    def __init__(
        ...
        if libc_path:
            libc_candidates.append(libc_path)

        if os.name == "nt":
            for win_name in ("ucrtbase", "msvcrt"):
                win_path = find_library(win_name)
                if win_path:
                    libc_candidates.append(win_path)
            libc_candidates.extend(["ucrtbase.dll", "msvcrt.dll"])

        for candidate in libc_candidates:
            try:
                libc = ctypes.cdll.LoadLibrary(candidate)
                break
            except OSError:
                continue
        ...
        if libc is None:
            try:
                ...
                raise InitializationError("Unable to find libc") from exc
